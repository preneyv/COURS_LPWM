...
public class FichierHTML extends Fichier{
     ...
     */
        public FichierHTML(String f) throws FileNotFoundException{
            super(f);
            String[] ext = f.split("\\.");
            this.ext = "html".equals(ext[1]) ? "html" : null;
        }

     /**
     * ouvrirFichier() test d'abord si il s'agit bien d'un fichier html ac l'attribut ext. Si ce n'est pas le cas, je signale une erreur avec rep
     ...
     * ligne du contenu contient bien ce qu'il faut écrire en début de fichier HTML.
     */
    @Override
    public void ouvrirFichier() {
        String rep="";
        if( "html".equals(this.ext)){ 
            
            super.ouvrirFichier();
            this.contenu = super.getContenu();
            rep = (this.contenu.size()>0 && this.contenu.get(0).equals("<!DOCTYPE Html>")) ? "Le fichier est prêt à être lu" : "Il y a un problème avec le fichier";
 
        }else{
            rep = "Le fichier que vous tentez d'ouvrir n'est pas un fichier .html";
        }
     ...
     * 
     * @return ret 
     * retourne dans une phrase le type de fichier. Sinon inconnu si ce n'est pas HTML.
     * Un peu inutile sachant que l'on est dans une classe de lecteur de fichier HTML. C'est 
     * plus pour la pratique et me remette dans le bain des opérateurs ternaires en java.
     */
    @Override
    public String typeFichier() {
        String ret;
        return ret = this.ext == "html" && this.contenu != null ? "C'est un fichier HTML" : "Type de fichier inconnu";
    }
