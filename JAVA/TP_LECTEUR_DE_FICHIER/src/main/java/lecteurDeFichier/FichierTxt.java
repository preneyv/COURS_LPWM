...
public class FichierTxt extends Fichier{
     ...
     */
        public FichierTxt(String f) throws FileNotFoundException{
            super(f);
            String[] ext = f.split("\\.");
            this.ext = "txt".equals(ext[1]) ? "txt" : null;
        }

        
     /**
     * ouvrirFichier() test d'abord si il s'agit bien d'un fichier txt ac l'attribut ext. Si ce n'est pas le cas, je signale une erreur avec rep
     ...
     * C'est de cela dont je ne suis pas sûr.
     * 
     * Je stocke ensuite un message  dans rep pour dire si l'ouverture se passe bien ou non. Si le contenu n'est pas nul.
     */
    @Override
    public void ouvrirFichier() {
        ...
        if( "txt".equals(this.ext)){ 
            ...
            rep = (this.contenu.size()>0) ? "Le fichier est prêt à être lu" : "Il y a un problème avec le fichier";
 
        }else{
            rep = "Le fichier que vous tentez d'ouvrir n'est pas un fichier .txt";
        }
     ...
     * 
     * @return ret 
     * retourne dans une phrase le type de fichier. Sinon inconnu si ce n'est pas TXT.
     * Un peu inutile sachant que l'on est dans une classe de lecteur de fichier TXT. C'est 
     * plus pour la pratique et me remette dans le bain des opérateurs ternaires en java.
     */
    @Override
    public String typeFichier() {
        String ret;
        return ret = this.ext == "txt" && this.contenu != null ? "C'est un fichier TXT" : "Type de fichier inconnu";
    }
